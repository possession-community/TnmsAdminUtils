...
public class CSay(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        foreach (var gameClient in SharedSystem.GetModSharp().GetIServer().GetGameClients(true, true))
        {
            ...
            gameClient.GetPlayerController()?
                .PrintToCenter(
                    Plugin.LocalizeStringForPlayer(gameClient, "Say.Broadcast.CSay", executor, message));
        }
    }
}
