...
public class Unfreeze(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        {
            Freeze.UnfreezePlayer(gameClient);
        }

        string executor = PlayerUtil.GetPlayerName(client);
        string targetName = targets.GetTargetName();
        Plugin.TnmsLogger.LogAdminAction(client, $"Admin {executor} unfroze {targetName}");
        ...
        foreach (var gameClient in SharedSystem.GetModSharp().GetIServer().GetGameClients())
        {
            ...
            gameClient.GetPlayerController()?
                .PrintToChat(
                    ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(gameClient, "Freeze.Broadcast.Unfrozen", executor, targetName));
        }
    }
}
