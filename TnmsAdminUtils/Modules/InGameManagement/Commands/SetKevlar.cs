...
public class SetKevlar(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    public override string CommandName => "setkev";
    public override string CommandDescription => "Sets a player's armor and optional helmet.";

    public override TnmsCommandRegistrationType CommandRegistrationType =>
        TnmsCommandRegistrationType.Client | TnmsCommandRegistrationType.Server;

    protected override ICommandValidator? GetValidator() => new CompositeValidator()
        .Add(new PermissionValidator("tnms.adminutil.management.ingame.command.setkev", true))
        .Add(new ArgumentCountValidator(2, true))
        .Add(new TargetValidator(1, true))
        .Add(new RangedArgumentValidator<int>(0, 100, 2, true));
    ...
    protected override ValidationFailureResult OnValidationFailed(ValidationFailureContext context)
    {
        switch (context.Validator)
        {
            case ArgumentCountValidator:
                PrintMessageToServerOrPlayerChat(context.Client, ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(context.Client, "SetKevlar.Notification.Usage"));
                break;
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        int armor = validatedArguments.GetArgument<int>(2);

        bool hasHelmet = false;
        if (commandInfo.ArgCount > 3 && int.TryParse(commandInfo.GetArg(3), out int helmetVal))
            hasHelmet = helmetVal == 1;

        foreach (var gameClient in targets)
        {
            PlayerUtil.SetPlayerArmor(gameClient, armor, hasHelmet);
        }

        string executor = PlayerUtil.GetPlayerName(client);
        string targetName = targets.GetTargetName();
        string helmetStr = hasHelmet ? " + helmet" : "";
        Plugin.TnmsLogger.LogAdminAction(client, $"Admin {executor} set {targetName} armor to {armor}{helmetStr}");
        ...
        foreach (var gameClient in SharedSystem.GetModSharp().GetIServer().GetGameClients(true, true))
        {
            ...
            gameClient.GetPlayerController()?
                .PrintToChat(
                    ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(gameClient, "SetKevlar.Broadcast.ArmorSet", executor, targetName, armor, helmetStr));
        }
    }
}
