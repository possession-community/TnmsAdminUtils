...
public class SetModel(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    ...
    protected override ValidationFailureResult OnValidationFailed(ValidationFailureContext context)
    {
        switch (context.Validator)
        {
            case ArgumentCountValidator:
                PrintMessageToServerOrPlayerChat(context.Client, ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(context.Client, "SetModel.Notification.Usage"));
                break;
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        string modelPath = commandInfo.GetArg(2);

        foreach (var gameClient in targets)
        {
            PlayerUtil.SetPlayerModel(gameClient, modelPath);
        }

        string executor = PlayerUtil.GetPlayerName(client);
        string targetName = targets.GetTargetName();
        Plugin.TnmsLogger.LogAdminAction(client, $"Admin {executor} set {targetName} model to {modelPath}");
        ...
        foreach (var gameClient in SharedSystem.GetModSharp().GetIServer().GetGameClients(true, true))
        {
            ...
            gameClient.GetPlayerController()?
                .PrintToChat(
                    ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(gameClient, "SetModel.Broadcast.ModelSet", executor, targetName, modelPath));
        }
    }
}
