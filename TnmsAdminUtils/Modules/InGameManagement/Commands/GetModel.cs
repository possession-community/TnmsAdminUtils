...
public class GetModel(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    public override string CommandName => "getmodel";
    public override string CommandDescription => "Shows a player's current model path.";
    ...
    protected override ValidationFailureResult OnValidationFailed(ValidationFailureContext context)
    {
        switch (context.Validator)
        {
            case ArgumentCountValidator:
                PrintMessageToServerOrPlayerChat(context.Client, ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(context.Client, "GetModel.Notification.Usage"));
                break;
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        {
            string name = PlayerUtil.GetPlayerName(gameClient);
            string model = PlayerUtil.GetPlayerModel(gameClient);
            PrintMessageToServerOrPlayerChat(client, ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(client, "GetModel.Notification.Result", name, model));
        }
    }
}
