...
public class WeaponGive(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    public override string CommandName => "give";
    public override string CommandDescription => "Gives a weapon to a player.";
    ...
    protected override ValidationFailureResult OnValidationFailed(ValidationFailureContext context)
    {
        switch (context.Validator)
        {
            case ArgumentCountValidator:
                PrintMessageToServerOrPlayerChat(context.Client, ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(context.Client, "WeaponGive.Notification.Usage"));
                break;
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        string weaponArg = commandInfo.GetArg(2).ToLower();

        if (!weaponArg.StartsWith("weapon_"))
            weaponArg = $"weapon_{weaponArg}";

        foreach (var gameClient in targets)
        {
            var pawn = gameClient.GetPlayerPawn();
            pawn?.GiveNamedItem(weaponArg);
        }

        string executor = PlayerUtil.GetPlayerName(client);
        string targetName = targets.GetTargetName();
        Plugin.TnmsLogger.LogAdminAction(client, $"Admin {executor} gave {weaponArg} to {targetName}");
        ...
        foreach (var gameClient in SharedSystem.GetModSharp().GetIServer().GetGameClients())
        {
            ...
            gameClient.GetPlayerController()?
                .PrintToChat(
                    ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(gameClient, "WeaponGive.Broadcast.WeaponGiven", executor, targetName, weaponArg));
        }
    }
}
