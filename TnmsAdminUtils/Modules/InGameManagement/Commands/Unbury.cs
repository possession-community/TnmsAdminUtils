...
public class Unbury(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    public override string CommandName => "unbury";
    public override string CommandDescription => "Unburies a player from the ground.";
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        foreach (var gameClient in SharedSystem.GetModSharp().GetIServer().GetGameClients())
        {
            ...
            gameClient.GetPlayerController()?
                .PrintToChat(
                    ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(gameClient, "Bury.Broadcast.Unburied", executor, targetName));
        }
    }
}
