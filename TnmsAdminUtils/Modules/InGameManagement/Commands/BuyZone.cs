...
public class BuyZone(IServiceProvider provider) : TnmsAbstractCommandBase(provider)
{
    public override string CommandName => "buyzone";
    public override string CommandDescription => "Toggles a player's buy zone access.";

    public override TnmsCommandRegistrationType CommandRegistrationType =>
        TnmsCommandRegistrationType.Client | TnmsCommandRegistrationType.Server;

    protected override ICommandValidator? GetValidator() => new CompositeValidator()
        .Add(new PermissionValidator("tnms.adminutil.management.ingame.command.buyzone", true))
    ...
    protected override ValidationFailureResult OnValidationFailed(ValidationFailureContext context)
    {
        switch (context.Validator)
        {
            case ArgumentCountValidator:
                PrintMessageToServerOrPlayerChat(context.Client, ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(context.Client, "BuyZone.Notification.Usage"));
                break;
    ...
    protected override void ExecuteCommand(IGameClient? client, StringCommand commandInfo, ValidatedArguments? validatedArguments)
    {
        ...
        bool inBuyZone = value == 1;

        foreach (var gameClient in targets)
        {
            PlayerUtil.SetPlayerBuyZoneStatus(gameClient, inBuyZone);
        }

        string executor = PlayerUtil.GetPlayerName(client);
        string targetName = targets.GetTargetName();
        string state = inBuyZone ? "ON" : "OFF";
        Plugin.TnmsLogger.LogAdminAction(client, $"Admin {executor} set {targetName} buy zone {state}");
        ...
        foreach (var gameClient in SharedSystem.GetModSharp().GetIServer().GetGameClients(true, true))
        {
            ...
            gameClient.GetPlayerController()?
                .PrintToChat(
                    ((TnmsAdminUtils)Plugin).LocalizeWithPluginPrefix(gameClient, "BuyZone.Broadcast.BuyZoneSet", executor, targetName, state));
        }
    }
}
